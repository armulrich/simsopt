...
# Ensure the local simsopt checkout (../src) is importable when running this
...
from simsopt.solve.macromagnetics import muse2tiles, MacroMag, SolverParams
...
def make_Bnormal_plots(field, surface, out_dir, tag):
    """Write a VTK surface file containing the coil/dipole B·n field.

    Older versions of the paper code used a helper with this name.  In current
    simsopt, we keep the post-processing self-contained by emitting a VTK file.
    """
    out_dir = Path(out_dir)
    pts = surface.gamma().reshape((-1, 3))
    normals = surface.unitnormal().reshape((-1, 3))
    field.set_points(pts)
    Bn = np.sum(field.B() * normals, axis=1).reshape(surface.gamma().shape[:2])
    vtk_path = out_dir / f"surface_Bnormal_{tag}"
    surface.to_vtk(vtk_path, extra_data={"Bnormal": reshape_to_vtk_field_format(Bn)})
    print(f"[SIMSOPT] Wrote {vtk_path}.vts with fields: Bnormal")
    return Bn
...
mac, A_mm = mac.direct_solve(
    ...
    params=SolverParams(use_coils=False, krylov_tol=1e-6, krylov_it=20, print_progress=False),  # No coil coupling
)
...
mac, A_full = mac.direct_solve(
    N_new_rows=N_full,  # Use full demagnetization tensor
    params=SolverParams(use_coils=True, krylov_tol=1e-6, krylov_it=20, print_progress=False),  # Include coil coupling
)
