...
if algorithm in ("GPMO_Backtracking", "GPMO", "GPMO_py", "GPMOmr"):
    ...
    params = GPMOParams(K=nIter_max, nhistory=nHistory)
...
if algorithm == "GPMOmr":
    ...
    mm_suffix = f"_kmm{params.mm_refine_every}"
...
t1 = time.time()
R2_history, Bn_history, m_history = GPMO(pm_opt, algorithm, params=params)
t2 = time.time()
...
K = int(getattr(params, "K", nIter_max))
nhistory = int(getattr(params, "nhistory", nHistory))

run_id = f"K{K}_nphi{nphi}_ntheta{ntheta}_ds{downsample}{full_suffix}_{algorithm}"
run_yaml = out_dir / f"run_{run_id}.yaml"
run_csv = out_dir / f"runhistory_{run_id}.csv"

record_every = int(getattr(pm_opt, "record_every", max(1, K // max(1, nhistory))))
k_hist = getattr(pm_opt, "k_history", None)
if k_hist is None:
    # Fallback: use a safe nominal schedule.
    k_hist = list(range(len(R2_history)))
else:
    k_hist = list(np.asarray(k_hist, dtype=int).ravel())
if len(k_hist) < len(R2_history):
    pad = int(k_hist[-1]) if k_hist else 0
    k_hist = k_hist + [pad] * (len(R2_history) - len(k_hist))
elif len(k_hist) > len(R2_history):
    k_hist = k_hist[: len(R2_history)]
...
if n_active is None:
    ...
    n_active = list(np.asarray(n_active, dtype=int).ravel())
    if len(n_active) < len(R2_history):
        pad = int(n_active[-1]) if n_active else 0
        n_active = n_active + [pad] * (len(R2_history) - len(n_active))
    elif len(n_active) > len(R2_history):
        n_active = n_active[: len(R2_history)]
...
run_doc = {
    ...
    "params": {
        "K": int(K),
        "nhistory": int(nhistory),
        "record_every": int(record_every),
        "history_every_requested": int(history_every),
        "save_plots": bool(save_plots),
        "vmec": bool(vmec_flag),
        "backtracking": int(getattr(params, "backtracking", 0)),
        "Nadjacent": int(getattr(params, "Nadjacent", 0)),
        "max_nMagnets": int(getattr(params, "max_nMagnets", 0)),
        "thresh_angle_rad": float(getattr(params, "thresh_angle", np.pi)),
        "mm_refine_every": int(getattr(params, "mm_refine_every", 0)),
        "use_coils": bool(getattr(params, "use_coils", False)),
        "use_demag": bool(getattr(params, "use_demag", False)),
        "coil_path": None if getattr(params, "coil_path", None) is None else str(getattr(params, "coil_path")),
        "cube_dim_m": float(getattr(params, "cube_dim", cube_dim)),
    },
    "artifacts": {
